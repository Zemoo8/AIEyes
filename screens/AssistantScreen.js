...
import {
  ...
  KeyboardAvoidingView, Platform, Pressable, ScrollView, Dimensions,
} from 'react-native';
...
const C = {
  ...
  ice:       '#f1f5ff',
  textPri:   '#F4F7FF',
  textSec:   'rgba(225,220,255,0.72)',
  textMuted: 'rgba(182,170,230,0.46)',
  border:    'rgba(178,155,255,0.18)',
  inputBg:   'rgba(14,14,30,0.62)',
};

const { width: SW, height: SH } = Dimensions.get('window');

const STARS = [
  { x: 0.08, y: 0.06, sz: 1.5 }, { x: 0.91, y: 0.04, sz: 2 },
  { x: 0.32, y: 0.10, sz: 1.5 }, { x: 0.72, y: 0.08, sz: 2 },
  { x: 0.18, y: 0.18, sz: 1.5 }, { x: 0.85, y: 0.14, sz: 1.5 },
  { x: 0.55, y: 0.05, sz: 2 },   { x: 0.44, y: 0.20, sz: 1.5 },
  { x: 0.04, y: 0.25, sz: 1.5 }, { x: 0.96, y: 0.22, sz: 2 },
  { x: 0.27, y: 0.28, sz: 1.5 }, { x: 0.78, y: 0.24, sz: 1.5 },
  { x: 0.62, y: 0.32, sz: 2 },   { x: 0.15, y: 0.35, sz: 1.5 },
];
...
// ══════════════════════════════════════════════════════════════════════════════
export default function AssistantScreen({ onBack, onSendQuery, lastAnswer, onMicTap, isListening }) {
  // state: 'idle' | 'listening' | 'thinking' | 'typing' | 'speaking' | 'answer'
  ...
  useEffect(() => {
    ...
    const blink = setInterval(() => setCursorOn(v => !v), 420);
    return () => clearInterval(blink);
  }, [state]);

  // sync visual state with parent's recording state (mic button or shake)
  useEffect(() => {
    if (isListening) {
      setState('listening');
    } else {
  ...
  async function send(text) {
    ...
    if (!t) return;
    setQuery(t); setInput(''); setAnswer(''); setDisplayed(''); setState('thinking');
    try {
      ...
      setState('typing'); // triggers typewriter → then speaking
    } catch (e) {
  ...
  function toggleMic() {
    // Fully delegate start/stop logic to parent (App.js handleMic).
    // Visual state is driven by isListening prop via the sync effect above.
    onMicTap?.();
  }
    ...
    <Animated.View style={[st.root, fadeStyle]}>
      ...
      {STARS.map((star, i) => (
        <View key={i} pointerEvents="none" style={{
          position: 'absolute',
          left: star.x * SW, top: star.y * SH,
          width: star.sz, height: star.sz, borderRadius: star.sz,
          backgroundColor: 'rgba(220,235,255,0.75)',
        }}/>
      ))}
      {/* bottom blue glow */}
      <View style={st.glow}/>
      ...
      >
        ...
        {/* ─── typing / speaking / answer — editorial answer panel ── */}
        {(state === 'typing' || state === 'speaking' || state === 'answer') && (
          <ScrollView contentContainerStyle={st.answerScroll} showsVerticalScrollIndicator={false}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 12, marginBottom: 18 }}>
              <AnimatedAssistantOrb size={56} state={state === 'answer' ? 'idle' : state}/>
              <View>
                <Text style={st.smallTag}>AIEYES</Text>
                {state === 'typing' && (
                  <Text style={st.stateChip}>TYPING · يكتب</Text>
                )}
                {state === 'speaking' && (
                  <Text style={[st.stateChip, { color: C.cyan }]}>SPEAKING · يتحدّث</Text>
                )}
              </View>
            </View>
...
const st = StyleSheet.create({
  ...
  stateChip: {
    color: '#b29bff', fontSize: 9, fontWeight: '700',
  ...
  inputPillActive: {
    borderColor: 'rgba(140,200,255,0.48)',
    backgroundColor: 'rgba(24,55,120,0.42)',
  },
  input: {
